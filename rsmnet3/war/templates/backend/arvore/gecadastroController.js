function gecadastroController($scope,$http,$routeParams,$location)
{
  ...
  $scope.cadastrar = function(){
				...
				$scope.retorno_error    = ''; 
				 ...
				 $http.get($scope.server("/gecadastro/pais")).success(function(data){
 			       $scope.registros_pais   = data;
  				  });
				
				
  				 $http.get($scope.server("/gecadastro/estados")).success(function(data){
 			       $scope.registros_estado   = data;
  				  });
				  
				  $http.get($scope.server("/gecadastro/planos")).success(function(data){
	...
	if(cd_estado === nome) {
	  return;
			...
			$http.get($scope.server("/gecadastro/cidades/"+cd_estado)).success(function(data){
	...
	$scope.$watch("capital",function( cd_estado, nome ) {
		if(cd_estado === nome) {
		  return;
				...
				$http.get($scope.server("/gecadastro/capitais/"+cd_estado)).success(function(data){
  ...
  $scope.cpfConta = function() {
		   ...
		   $http.get($scope.server("/gecadastro/cpf/"+$scope.cpf)).success(function(data){
  ...
  $scope.cnpjConta = function() {
		   ...
		   $http.get($scope.server("/gecadastro/cnpj/"+$scope.cnpj)).success(function(data){
...
$scope.salvar = function() {
	...
	$http({
        method  : 'POST',
        url     : $scope.server("/gecadastro/salvar"),
        data    : dados,  
