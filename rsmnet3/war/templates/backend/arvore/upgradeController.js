function upgradeController($scope,$http,$routeParams,$location)
{
...
$scope.upgrade = function(){
		...
		$http.get($scope.server("/upgrade/Json")).success(function(data){
...
$scope.newPlano = function(plano) {
  	 $http.get($scope.server("/upgrade/consulta/"+plano)).success(function(data){
...
$scope.upgradeSalvar = function() {
	...
	$http({
		method  : 'POST',
		url     : $scope.server("/upgrade/salvar"),
		data    : dados,  
