...
import br.com.ibracon.idr.activity.AnotacoesActivity;
import br.com.ibracon.idr.activity.EscolherListaActivity;
...
//	private ImageView btn_note;
	...
	private ImageView btn_escolha_lista;
	private ImageView btn_anotacoes;
...
//	private ImageView btn_ir;
	...
	private ResponseIndice responseIndice;
	
	private Livro livro;

	public final static String LIVRO = "br.com.ibracon.chave_extra_pdf_caminho_arquivo";
	public final static String EXTRA_PDF_SENHA = "br.com.ibracon.chave_extra_pdf_senha_arquivo";
	public final static String RESPONSE_INDICE_OBJ = "br.com.ibracon.idr.indice.ResponseIndice";
	public final static String NUMERO_PAGINA = "br.com.ibracon.idr.acitivity.PDFReaderAct.NUMERO_PAGINA";
...
//		btn_note = (ImageView) m_layout.findViewById(R.id.btn_note);
		btn_escolha_lista = (ImageView) m_layout.findViewById(R.id.btn_escolher_lista);
		btn_anotacoes = (ImageView) m_layout.findViewById(R.id.btn_anotacoes);
//		btn_ir = (ImageView) m_layout.findViewById(R.id.btn_ir);
		...
		btn_prev = (ImageView) bar_find.findViewById(R.id.btn_prev);
		btn_next = (ImageView) bar_find.findViewById(R.id.btn_next);
...
//		btn_note.setOnClickListener(this);
		btn_escolha_lista.setOnClickListener(this);
...
//		btn_ir.setOnClickListener(this);
		...
		livro = (Livro) bundle.get(LIVRO);
		String password = bundle.getString(EXTRA_PDF_SENHA);
		responseIndice = (ResponseIndice) bundle.get(PDFReaderAct.RESPONSE_INDICE_OBJ);
		m_doc = new Document();
		int retorno = m_doc.Open(livro.getArquivomobile(), password);
		m_reader.PDFOpen(m_doc, false, this);
		m_thumb.thumbOpen(m_doc, this);
...
//		btn_note.setEnabled(m_reader.PDFCanSave());
	...
	private void abrirEscolhaLista(){
		// REDIRECIONANDO PARA O DETALHAMENTO DO LIVRO
		Intent intent = new Intent(this,
				EscolherListaActivity.class);
		intent.putExtra(RESPONSE_INDICE_OBJ, responseIndice);
		intent.putExtra(LIVRO, livro);
		startActivityForResult(intent, 1);
	}
	
	private void abrirAnotacoesPagina(){
		Intent intent = new Intent(this,
				AnotacoesActivity.class);
		Nota nota = new Nota();
		nota.setCodigoLivro(livro.getCodigolivro());
		nota.setPagina(String.valueOf(paginaAtual));
		
		intent.putExtra(AnotacoesActivity.NOTA, nota);
		startActivityForResult(intent, 1);
	}
...
//			onNote();
			...
			onFindNext();
		else if (v.getId() == R.id.btn_escolher_lista)
			abrirEscolhaLista();
		else if (v.getId() == R.id.btn_anotacoes)
			abrirAnotacoesPagina();
...
//		btn_note.setEnabled(annot == null && m_reader.PDFCanSave());
	...
	public void onActivityResult(int requestCode, int resultCode,
		Intent data) {
		try{
			int numeroPagina = data.getExtras().getInt(NUMERO_PAGINA);
			m_reader.PDFGotoPage(numeroPagina-1);
		}catch(Exception e){
		}
	}
}
